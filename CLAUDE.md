...
**Сдвиг вёрстки от картинок — `node web/scripts/check_layout_shift.mjs`** (без аргумента — прод,
с URL — локальная сборка): CLS и LCP по шаблонам «с картинкой» против контрольных «без», плюс
проверка, что у каждого `<img>` заданы `width`/`height`. Гонять при любом изменении шапки
сущности или добавлении картинок в новый шаблон. Место под портрет резервируют ОБА слоя —
grid-колонка в `EntityHead.astro` (168/92 px) и атрибуты размеров, поэтому CLS держится на нуле
даже если один слой убрать; убирать всё равно нельзя, атрибуты нужны до применения CSS.

Два подвоха замера, оба ловил на себе: CDP-троттлинг привязан к вкладке (одна сессия соседние
страницы не замедляет), а общий контекст между страницами тащит зарегистрированный service
worker, который дальше отдаёт всё из Cache Storage — LCP выходил 76 мс «по Fast 3G» вместо
честных ~800. Поэтому в скрипте на каждую страницу свой контекст и свой троттлинг. И PSI сюда
не годится: у публичного API общая анонимная квота, регулярно исчерпанная.
