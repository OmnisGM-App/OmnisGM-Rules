...
**исходников**: `scripts/build_fingerprint.mjs` пишет его в `dist/build-id.txt` на сборке
(postbuild) и пересчитывает в страже. Входы — `web/src`, `web/public`, контент `src/` в корне
репозитория, плюс `astro.config.mjs` и `package.json` целиком. Считается за ~160 мс.
