...
Чего страж не умеет: различать серверы из ОДНОГО каталога. Свой `npm run dev`, окажись он на
порту preview, был бы переиспользован молча — поэтому у dev и заведена своя база порта.
Отличать свежую сборку от устаревшей можно только по отпечатку `dist` — отдельная задача.
