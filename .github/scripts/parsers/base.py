...
def extract_names(heading: str, lang: str) -> tuple[str, str | None, str]:
    ...
    if lang != "en":
        ...
        # (Luckstone))»: не-жадный захват до первой ЛАТИНСКОЙ скобки + один уровень вложенности.
        # После EN-имени допускаются ХВОСТОВЫЕ квалификаторы-скобки (кириллица), которых нет в 5.2,
        # но есть в 5.1: «Балор (Balor) (Демон)», «Взрослый бронзовый дракон (Adult Bronze Dragon)
        # (Металлический)». Их держим в отображаемом RU-имени (симметрично EN, где «(Metallic)»
        # тоже остаётся в имени), а слаг/name_en берём из английской скобки → EN/RU слаги совпадут.
        # Хвостовые квалификаторы — только НЕ-латинские скобки (кириллица: «(Демон)»,
        # «(Металлический)»). Латинская вторая скобка — часть английского имени, не хвост.
        m = re.match(
        ...
        if m:
            ...
            name_local = (m.group(1).strip() + (" " + trail if trail else "")).strip()
