...
Уникальное покрытие даёт ОДНА форма — граница диапазона («Huge or Smaller», RU «Огромный
или меньший»): она из врезки, которой не видит ни один соседний гейт, и её откат оставляет
гейты шапок и полей зелёными. Составной тип («Large Celestial, Fey, or Fiend (Your Choice),
Neutral») и запятая ВНУТРИ скобок подтипа уникальными быть перестали: с #290 разрез шапки
общий, и гейт шапок держит обе формы таблицей самопроверок (`SPLIT_CASES`) — откат склейки
по последней запятой и снятие отрицательного просмотра красят все три гейта. Остальные
формы держит и гейт шапок.
...
SYNTHETIC = {
    ...
    # ВЕРХНЯЯ граница: её в корпусе нет вовсе (#303), а парсер знает с #260. Мужской и
    # средний род — единственные, где эта форма проверяема: женская «большая» лежит и в
    # словаре размеров (Large), поэтому разрез сходится и БЕЗ участия набора `bounds`, то
    # есть женским рядом снос «больший»/«большее» из парсера не поймать (ревью #317).
    "*Огромный или больший Конструкт, без мировоззрения*",
    "*Огромное или большее чудовище, без мировоззрения*",
}
CASES = [
    ...
    # Верхняя граница — те же два рода, и по той же причине: они единственные пришпинивают
    # набор `bounds` продукционного парсера, женский род его не касается (#303).
    ("*Огромный или больший Конструкт, без мировоззрения*", "ru",
     "Огромный или больший", "Конструкт", None, "без мировоззрения"),
    ("*Огромное или большее чудовище, без мировоззрения*", "ru",
     "Огромное или большее", "чудовище", None, "без мировоззрения"),
