...
Уникальное покрытие дают ДВЕ формы, и обе — из блоков, которых не видит ни один соседний
гейт: граница диапазона («Huge or Smaller», RU «Огромный или меньший») во врезке
заклинания и запятая ВНУТРИ скобок подтипа у 5.1 («Fiend (Devil, Shapechanger), Lawful
Evil») — откат любой из них оставляет `test_statblock_headers.py` и `test_statblock_fields.py`
зелёными. Остальные формы держит и гейт шапок; здесь они стоят как контракт функции.
