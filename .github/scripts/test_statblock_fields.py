...
SIDEBAR_CASES = [
    ...
    ("подменена оговорка", "Large Celestial, Fey, or Fiend (Your Choice), Neutral",
     ...
     ["'на выбор Мастера'"]),
    # Граница диапазона: связка — часть РАЗМЕРА, и её род согласуется с типом. Носителя
    # у этой сверки не было ни у одного гейта (единственная живая форма — врезка Animate
    # Objects, мужской род), поэтому «меньшая» вместо «меньший» и наоборот проходили
    # молча (#294).
    ("граница диапазона", "Huge or Smaller Construct, Neutral Evil",
     "Огромный или меньший Конструкт, Нейтрально-злой", []),
    ("граница диапазона, женский род", "Huge or Smaller Undead, Neutral Evil",
     "Огромная или меньшая Нежить, Нейтрально-злой", []),
    ("граница диапазона: род связки не согласован", "Huge or Smaller Undead, Neutral Evil",
     "Огромная или меньший Нежить, Нейтрально-злой",
     ["граница диапазона «меньший» не согласована с «Нежить»"]),
    # ВЕРХНЯЯ граница (#303). Женский род — не случайный выбор: только у него связка
    # («большая») совпадает со словарным размером Large по всему, кроме регистра. До #303
    # разбор врезки отдавал её размеру, а `SERVICE["en"]` не знал `Larger`, и EN-половина
    # отдавала «?»-токен: сверка перевода целиком гасилась веткой «сверять не с чем» —
    # то есть проверка не краснела, а молча выключалась (ревью #317).
    ("верхняя граница диапазона", "Huge or Larger Undead, Neutral Evil",
     "Огромная или большая Нежить, Нейтрально-злой", []),
    ("верхняя граница: род связки не согласован", "Huge or Larger Undead, Neutral Evil",
     "Огромная или больший Нежить, Нейтрально-злой",
     ["граница диапазона «больший» не согласована с «Нежить»"]),
    # Обратная сторона: с ПРОПИСНОЙ это честный диапазон из двух размеров. Ряд держит то,
    # что развилка по регистру не съела законную форму — без него перестановка веток
    # выглядела бы бесплатной.
    ("диапазон из двух размеров с омонимом", "Huge or Large Undead, Neutral Evil",
     "Огромная или Большая Нежить, Нейтрально-злой", []),
    # Регистр связки испорчен. У «меньш*» омонима среди размеров нет вовсе (Small —
    # «Маленький»), поэтому «Меньший» опознаётся связкой в любом регистре и краснеет как
    # несогласованный род — сообщение указывает на настоящий дефект.
    ("регистр связки границы", "Huge or Smaller Undead, Neutral Evil",
     "Огромная или Меньший Нежить, Нейтрально-злой",
     ["граница диапазона «Меньший» не согласована с «Нежить»", "'Меньший'"]),
    # А у «больш*» омоним есть, и потому регистр там опознаёт СЛОВО, а не только согласие:
    # «БОЛЬШАЯ» — это испорченный регистр РАЗМЕРА Large, и назвать его связкой значило бы
    # посоветовать правку, которая меняет смысл шапки на верхнюю границу (ревью #317).
    ("мис-регистр размера в диапазоне", "Huge or Large Undead, Neutral Evil",
     "Огромная или БОЛЬШАЯ Нежить, Нейтрально-злой",
     ["размер «БОЛЬШАЯ» не согласован с «Нежить» — ожидалось «Большая»"]),
    # Средний род связки не был пришпинен НИ У ОДНОЙ границы: все ряды строились на
    # «Конструкт» (муж.) и «Нежить» (жен.), и обрезанный с хвоста кортеж `BOUND_FORMS`
    # оставлял оба гейта зелёными — а на живой шапке дал бы IndexError посреди прогона.
    ("граница диапазона, средний род", "Huge or Smaller Plant, Neutral Evil",
     "Огромное или меньшее Растение, Нейтрально-злой", []),
    ("верхняя граница, средний род", "Huge or Larger Plant, Neutral Evil",
     "Огромное или большее Растение, Нейтрально-злой", []),
