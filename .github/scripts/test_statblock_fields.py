...
SIDEBAR_CASES = [
    ...
    # («большая») совпадает со словарным размером Large по всему, кроме регистра, и до #303
    # разбор врезки отдавал её размеру. EN-половина при этом говорит LARGER, RU — Large,
    # то есть верная врезка объявлялась расхождением.
    ...
    ("диапазон из двух размеров с омонимом", "Huge or Large Undead, Neutral Evil",
     "Огромная или Большая Нежить, Нейтрально-злой", []),
    # Регистр связки испорчен: «Меньший» — не размер (у Large форма «Большой»), поэтому
    # он обязан остаться неопознанным словом и разойтись с EN-половиной, а не молча
    # сойти за связку.
