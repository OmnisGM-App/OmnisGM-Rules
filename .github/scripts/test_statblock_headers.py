...
# (например, её отступили пробелами) — это дефект, а не отсутствие данных: гейтить по
# пустоте карты значило бы выключать сверку ровно тем входом, от которого она защищает.
SUBTYPES_KNOWN = SUBTYPE_DICT.exists()
...
# пропускает равномерную порчу всего места разом; носителей много — 317 шапок 5.1,
# 330 шапок 5.2 и по строке указателя на каждую.
ALIGN_CASE = {
...
def check_version(version: str, fixture: Path) -> None:
    ...
    # шапок из 317. Проверяем две вещи. ЕДИНСТВО внутри версии: правка одной строки
    # делает набор смешанным и краснеет. И САМУ КОНВЕНЦИЮ редакции — иначе равномерная
    # порча всей главы разом (все 317 шапок 5.1 со строчной) оставляет набор однородным и
    # проходит молча, а красит её гейт полей, где форма врезки сверяется с главой:
    # виноватой оказывается правильная врезка (#271, ревью #281).
    for _what, _values in (("шапках", [h.rpartition(", ")[2].strip() for h in ru_headers.values()]),
        ...
        if want_upper is None:
            failures.append(f"{version}: конвенция регистра мировоззрения в {_what} не "
                            ...
                            f"перевода")
        elif _values and bool(_upper) != want_upper:
            failures.append(
                f"{version} RU: мировоззрение в {_what} пишется "
                f"{'со строчной' if _lower else 'с прописной'} во всех {len(_values)} "
                f"строках, а конвенция этого места — "
                f"{'с прописной' if want_upper else 'со строчной'} "
                f"(например «{sorted(set(_values))[0]}»)")
    ...
    for index, align_col in (("04_Monsters.md", 4), ("05_Animals.md", None)):
        ...
        # «Правили шапку» — это когда ВСЕ строки таблицы одной ширины, и она не сходится
        # с шапкой. Несколько сломанных строк при целой таблице так не выглядят, и
        # поимённость для них важнее краткости.
        if wrong_width and len(wrong_width) == len(rows) and len({n for _, n in wrong_width}) == 1:
            failures.append(
                f"{version} RU-указатель {index}: все {len(rows)} строк шириной "
