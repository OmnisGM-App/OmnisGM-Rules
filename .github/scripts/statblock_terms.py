...
def is_bound_ru(word: str) -> bool:
    ...
    Форма опознаётся без учёта регистра — иначе «Меньший» стал бы не несогласованной связкой,
    а неизвестным типом, и сообщение уводило бы не туда. Исключение одно и оно же причина
    существования функции: ТОЧНАЯ прописная форма размера — это размер. «Большая» = словарный
    Large, «большая» = верхняя граница; других различий у этой пары нет (#303).
    """
    if word in _SIZE_ANY_FORM:
        return False
    return word.lower() in BOUND_RU
...
def skeleton(expr: str, lang: str, types: dict, subtypes: dict, sizes_ru: dict) -> list:
    ...
    for token in re.findall(r"⟪TERM⟫|⟪PAREN⟫|[A-Za-zА-Яа-яЁё]+|\S", rest):
        ...
        elif lang == "ru" and token in BOUND_RU:
            # Строчная связка границы обгоняет размеры по той же причине, что и в
            # `size_agreement`: «большая» — верхняя граница, «Большая» — размер Large (#303).
            # Совпадение здесь ТОЧНОЕ: испорченный регистр связки («Меньший») обязан остаться
            # «?»-токеном и разойтись с EN-половиной, как любое чужое слово в шапке.
...
# Все словарные формы размера как они пишутся в шапке — с прописной. Нужен `is_bound_ru`
# (объявлен выше, зовётся после импорта модуля целиком): по этому набору он отличает размер
# «Большая» от связки верхней границы «большая» (#303).
