...
#
# ВЕРХНЯЯ граница («or Larger») заведена впрок (#303): носителя в корпусе нет ни одного, но
# продукционный парсер её разбирает с #260, и первая же такая шапка обвинила бы в неверном
# разборе ВЕРНЫЙ парсер. Её RU-форма — переводческое решение, записанное в
# `src/dnd/translate/logs/2026-09-09_log.md`: «больший / большая / большее», со строчной.
#
# У женской формы верхней границы есть омоним — словарный размер Large: «большая» = «Большая»
# с точностью до регистра (мужская и средняя расходятся: больший ≠ Большой). Разводит их
# РЕГИСТР, и это уже принятая конвенция шапок, а не уловка: размер пишется словарным термином
# с прописной (#256), связка — со строчной, как «или». Отсюда «Огромная или Большая Нежить» —
# честный диапазон из двух размеров, «Огромная или большая Нежить» — верхняя граница.
...
def is_bound_ru(word: str) -> bool:
    """Слово — связка границы диапазона («меньший», «большая»), а не размер.

    Регистр учитывается ровно там, где есть омоним. У «меньш*» омонима среди размеров нет
    (Small — «Маленький»), поэтому форма опознаётся в любом регистре: иначе «Меньший» стал бы
    не несогласованной связкой, а неизвестным типом, и сообщение уводило бы не туда. А у
    «больш*» омоним есть — словарный размер Large, — и там сравнение ТОЧНОЕ: «большая» связка,
    «Большая» размер, «БОЛЬШАЯ» — испорченный регистр РАЗМЕРА, а не связка.

    Регистронезависимость на всю таблицу (первая редакция #303) как раз этот случай и путала:
    «Огромная или БОЛЬШАЯ Нежить» диагностировалась как несогласованная связка, а совет
    «ожидалось „большая“» менял смысл шапки на верхнюю границу — два прогона давали два
    взаимоисключающих требования, и ни одно не называло настоящий дефект (ревью #317).
    """
    if word in _SIZE_ANY_FORM:
        return False
    low = word.lower()
    if low not in BOUND_RU:
        return False
    # У связки, чья форма омонимична размеру, регистр — часть опознания.
    if low in _SIZE_ANY_FORM_LOWER:
        return word == low
    return True
...
SERVICE = {
    "en": {"or": "OR", "smaller": "SMALLER", "larger": "LARGER", "and": "AND", "any": "ANY"},
...
def skeleton(expr: str, lang: str, types: dict, subtypes: dict, sizes_ru: dict) -> list:
    ...
    for token in re.findall(r"⟪TERM⟫|⟪PAREN⟫|[A-Za-zА-Яа-яЁё]+|\S", rest):
        ...
        elif lang == "ru" and is_bound_ru(token) and token in SERVICE["ru"]:
            # Связка границы обгоняет размеры по той же причине, что и в `size_agreement`:
            # «большая» — верхняя граница, «Большая» — размер Large (#303). Предикат ОБЩИЙ с
            # гейтом шапок: два места, решающих одно, разъезжаются.
            #
            # Второе условие — не украшение. Членство читалось по `BOUND_RU`, а значение
            # бралось из `SERVICE["ru"]` — независимой таблицы: добавь форму в одну и забудь
            # во второй, и гейт полей падал бы `KeyError` посреди обхода, унося весь уже
            # накопленный отчёт. С охраной тот же рассинхрон даёт «?»-токен и читаемую
            # строку расхождения (ревью #317).
...
# Все словарные формы размера как они пишутся в шапке — с прописной, и они же в нижнем
# регистре. Нужны `is_bound_ru` (объявлен выше, зовётся после импорта модуля целиком): по
# первому набору он отличает размер «Большая» от связки «большая», по второму — узнаёт, что у
# этой связки омоним вообще есть, и включает точное сравнение регистра (#303, ревью #317).
_SIZE_ANY_FORM = {form for forms in SIZE_FORMS.values() for form in forms}
_SIZE_ANY_FORM_LOWER = {form.lower() for form in _SIZE_ANY_FORM}
...
def size_agreement(header: str, types_ru, sizes_ru: dict):
    ...
    while i < len(words):
        # Связка границы проверяется ПЕРЕД размерами: у верхней границы женская форма
        # совпадает с размером Large по всему, кроме регистра, и порядок «сначала размеры»
        # съедал бы её как «Большая» с испорченным регистром (#303).
        if is_bound_ru(words[i]) and sizes:
            smaller.append(words[i]); i += 1
        elif words[i].lower() in sizes_ru:
