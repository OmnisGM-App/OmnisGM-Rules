...
// `doc-hub` действительно перестраиваются по ширине, а `404` попал сюда не поэтому — он
// носитель тега `@cross-engine` для WebKit-смоука (страница 404 вообще не использует
// ReaderShell). В `doc-hub` заодно едут проверки canonical/hreflang/sitemap и JSON-LD,
// которые от ширины не зависят: цена — несколько секунд, зато файл не приходится дробить
// (ревью #302).
const MOBILE_SPECS = [
  /reader\.spec\.ts/,
  /doc-hub\.spec\.ts/,
