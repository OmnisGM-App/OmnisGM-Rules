...
#
# ── Про CSP
# С #225 политика в ENFORCE. Собрана из того, что страницы реально грузят: свои бандлы и
# инлайновые модули Astro, шрифты, Pagefind с воркером и wasm, GA4, Метрика с вебвизором
# (включая её websocket) и beacon Cloudflare Web Analytics, который CF вставляет в HTML на
# эдже. Инлайн-скриптов у Astro много и хэши менялись бы каждый билд — поэтому
# `'unsafe-inline'`; смысл политики в том, чтобы запретить ЧУЖИЕ источники, а не инлайн.
# Инвентаризация источников — `node web/scripts/check_csp.mjs` (по проду: часть источников
# существует только на эдже). Консоль для этого не годится: нарушения пишет сам браузер,
# и через API чтения консоли они не видны.
set -u
