...
const BUDGET = {
  ...
  // #214: волна 1 (оружие + навыки BRP) 774 → 528, волна 2 (хабы, термины rules-glossary,
  // доспехи, снаряжение, каскад заклинаний) → 59. Остаток — хабы ровно с ОДНОЙ сущностью
  // (имя в сниппет не идёт: «Среди них: Аболет.» читается как обрывок) и несколько черт с
  // однострочным описанием. Дальше выжимать нечего без выдумывания фактов.
  // #196: 60 → 62. У 5.2 появился тип «Swarm» (шапка роя восстановлена по PDF: «Medium
  // Swarm of Tiny Undead» вместо «Medium Undead»), и вместе с ним два хаба —
  // monsters-a-z/type/swarm на EN и RU. Хаб ровно с одной сущностью и есть тот самый
  // неустранимый остаток выше, просто на две страницы больше.
